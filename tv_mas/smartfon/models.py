...
class SmartfonBrend(models.Model):
    ...
    def absolute_url(self):
        return reverse('smartfon:smartfon_home_and_search', kwargs={'brend': self.brend})



class SmartfonModels(models.Model):
    brend_key = models.ForeignKey(SmartfonBrend, blank=False, null=True, related_name = "smartfon_brend")
    ...
    @property
    def absolute_url(self):
        return reverse('smartfon:smartfon_home_and_search', kwargs={'model': self.model})
...
class Spare_Part(models.Model):
    smartfonmodels  = models.ForeignKey(SmartfonModels, blank=True, null=True, related_name = "smarfon_models")
    action_problem  = models.ForeignKey(ActionProblem,  blank=True, null=True, verbose_name="Категория проблем")
    name_service    = models.CharField("Название Услуги", unique=True, max_length=200, blank=True, null=True, default=None)
    ...
    all_smartfon    = models.BooleanField("Включить услугу для всех моделей", default=False)
    ...
    def __unicode__(self):
        return "%s**     **%s     **%s" % (self.smartfonmodels, self.popular_service, self.name_service) 
    ...
    @property
    def absolute_url(self):
        return reverse('smartfon:smartfon_home_and_search', kwargs={'name_service': self.name_service})
...
class Orders(models.Model):
    ...
    order_problem_name = models.CharField("Название услуги ", max_length=200, blank=True, null=True)
    order_problem_id   = models.CharField("Артикул: услуги", max_length=200, blank=True, null=True)
