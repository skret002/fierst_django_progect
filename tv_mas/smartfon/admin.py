...
class SmartfonBrendAdmin (admin.ModelAdmin):
    list_display = [field.name for field in SmartfonBrend._meta.fields]
    class Meta:
        model = SmartfonBrend
admin.site.register(SmartfonBrend, SmartfonBrendAdmin)

class SmartfonModelsAdmin (admin.ModelAdmin):
    list_display = [field.name for field in SmartfonModels._meta.fields]
    class Meta:
        model = SmartfonModels
admin.site.register(SmartfonModels, SmartfonModelsAdmin)


class Spare_PartAdmin (admin.ModelAdmin):
    list_display = [field.name for field in Spare_Part._meta.fields]
    list_filter = ('smartfonmodels__model', 'action_problem__name_service')
    search_fields = ['smartfonmodels__model', 'name_service','description_service',]
