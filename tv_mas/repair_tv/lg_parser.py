...
BASE_URL_LG = 'http://www.repair-tv-lg.ru/models'
atach_url_LG = 'https://www.repair-tv-lg.ru'
ROOT_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
BASE_DIR = os.path.join(ROOT_DIR, "templates","static", "media")
IMG_LG = os.path.join(BASE_DIR, "img/",  "lg/")
BREND_LG = "Lg"

def get_page_count_lg():
    response = requests.get(BASE_URL_LG).text
...
def parse_lg(url):
    ...
    Tv_Write_Info = Tv_Models.objects.update_or_create(number_tv_model=model_info[0]['name_models'],
                                                   ...
                                                   is_brend = BrendTv.objects.get_or_create(brend=BREND_LG)[0]
                                                   )
#def main_lg():
