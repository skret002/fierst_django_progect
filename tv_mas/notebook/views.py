...
def notebook_home_and_search(request):
    args ={}
    smarfon_data = []
    json_dict = dict()
    if request.method == 'GET'  and request.GET.get("input_model"):
        ...
        print("S MODEL", search_model)
        args['search_model'] = search_model
        if search_model:
            choise_models_bd = NotebookModels.objects.all().filter(model__icontains=search_model) 
            print(choise_models_bd)
    ...
    return render(request, 'notebook/remont_notebook.html',args)
    

def notebook_search_result(request):
    json_dict = dict()
...
@csrf_exempt
def NotebookService(request, model_name, model_id, brend=None, page_number=1):
    args    ={}
    services_and_price = Spare_Part.objects.all().filter(notebookmodels=model_id, autocheck=True)
    sale    = Spare_Part.objects.all().filter(sale=True) 
    ...
    product_s = Spare_Part.objects.all().filter(notebookmodels=model_id)
    ...
    if request.method == 'POST' and request.POST.get("choices_available"):
        ...
        product_sort = Spare_Part.objects.filter(notebookmodels=model_id, **sort )
    ...
    if request.method == 'POST' and request.POST.get("phone_number"):
        ...
        if form_order.is_valid():
            ...
            send_mail(u'ТВ-МАСТЕРСКАЯ ВОПРОС ПО РЕМОНТУ ТВ', u'Оформлен новый заказ на ремонт ноутбука  & моноблока.', settings.EMAIL_HOST_USER, ['79219032885@ya.ru'])
    ...
    for page in product.page_range:
        ...
        current.append({'url' : 'http://'+str(host)+'/'+'remont_notebook'+'/'+str(brend)+'/'+ str(model_name)+'/'+str(model_id)+'/'+str(page)+'/',
                        'page_num': str(page),
                        'prev_url': 'http://'+str(host)+'/'+'remont_notebook'+'/'+str(brend)+'/'+ str(model_name)+'/'+str(model_id)+'/'+str(prev_url)+'/',
                        'next_url': 'http://'+str(host)+'/'+'remont_notebook'+'/'+str(brend)+'/'+ str(model_name)+'/'+str(model_id)+'/'+str(next_url)+'/',})
    ...
    return render(request, 'notebook/service_notebook.html',args)

def PartsCart(request, model_name, model_id, problem, name_part = None):
    args ={}
    product = Spare_Part.objects.filter(notebookmodels=model_id, name_service=problem)
    ...
    return render(request, 'notebook/parts_cart.html',args)
...
def SalePartsCart(request, name_part):
    ...
    args['dopinfoservice'] = dopinfoservice
    args['product'] = product
    return render(request, 'notebook/sale_part_cart.html',args)